...
// 
// Related Topics 深度优先搜索 广度优先搜索 图 哈希表 👍 412 👎 0
...
class Solution {
    ...
    public void dfs(Node node){
    	// open表存储待遍历节点，先加进来再遍历
		Stack<Node> open = new Stack<>();
		// close保存一个节点是否被访问过
		HashMap<Node, Integer> close = new HashMap<>();
		// 首先加入初始节点
		open.push(node);
		while (!open.empty()){
			Node node1 = open.pop();
			...
			for(Node node2 : neighbors){
				if(close.getOrDefault(node2, 0)==0){
					open.push(node2);
					close.put(node2, 1);
				}
			}
	...
	public void dijistra(Node node){
